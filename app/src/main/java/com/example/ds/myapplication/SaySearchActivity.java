...
public class SaySearchActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
            ...
            public void onInit(int status) {
                if (status == TextToSpeech.SUCCESS) {
                    ...
                    if (result == TextToSpeech.LANG_MISSING_DATA || result == TextToSpeech.LANG_NOT_SUPPORTED) {
                        Toast.makeText(SaySearchActivity.this, "이 언어는 지원하지 않습니다.", Toast.LENGTH_SHORT).show();
                    } else {
        ...
        handler.postDelayed(new Runnable() {
            @Override
            public void run() {
                String sentence = "검색 서비스입니다.검색어를 먼저 말씀하시고 검색해줘 라고 이어붙이시면 해당 단어를 검색해드리고,실시간이라고 말씀하시면 실시간 순위를 알려드립니다.실시간 몇 위라고 말씀하시면 해당 순위의 헤드라인을 읽어드립니다.소리가 들리면 말씀해주세요 ";
